...
module NcsNavigator::Warehouse::Models::ThreePointTwo
  class BreastMilkKitDist
    include DataMapper::Resource
    include NcsNavigator::Warehouse::Models::MdesModel

    storage_names[:default] = 'breast_milk_kit_dist'
    ...
    property   :breast_milk_kit_dist_id,
               NcsNavigator::Warehouse::DataMapper::NcsString,
    ...
    property   :kit_collection_time,
               NcsNavigator::Warehouse::DataMapper::NcsString,
               { :required => true, :length => 1..2, :set => ["1", "2", "-3", "-4"] }
    property   :kit_distrib_method,
               NcsNavigator::Warehouse::DataMapper::NcsString,
               { :required => true, :length => 1..2, :set => ["1", "2", "3", "-3", "-4"] }
    property   :kit_distrib_reas,
               NcsNavigator::Warehouse::DataMapper::NcsString,
               { :required => true, :length => 1..2, :set => ["1", "2", "3", "-5", "-3", "-4"] }
    property   :kit_distrib_reas_oth,
               NcsNavigator::Warehouse::DataMapper::NcsString,
               { :pii => :possible, :length => 0..255 }
    property   :kit_distrib_date,
               NcsNavigator::Warehouse::DataMapper::NcsString,
               { :length => 0..10, :format => /^([0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9])?$/ }
    property   :kit_id,
               NcsNavigator::Warehouse::DataMapper::NcsString,
               { :length => 0..9 }
    property   :kit_collection_date,
               NcsNavigator::Warehouse::DataMapper::NcsString,
               { :length => 0..10, :format => /^([0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9])?$/ }
    belongs_to :staff,
               'NcsNavigator::Warehouse::Models::ThreePointTwo::Staff',
               :child_key => [ :staff_id ], :required => true
    property   :kit_form_comp_date,
               NcsNavigator::Warehouse::DataMapper::NcsString,
               { :length => 0..10, :format => /^([0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9])?$/ }
    property   :kit_comments,
               NcsNavigator::Warehouse::DataMapper::NcsText,
               { :pii => :possible, :length => 0..8000 }

    mdes_order :psu_id, :breast_milk_kit_dist_id, :p_id, :event_id, :kit_collection_time, :kit_distrib_method, :kit_distrib_reas, :kit_distrib_reas_oth, :kit_distrib_date, :kit_id, :kit_collection_date, :staff_id, :kit_form_comp_date, :kit_comments
