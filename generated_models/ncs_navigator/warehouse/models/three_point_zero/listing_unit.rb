...
module NcsNavigator::Warehouse::Models::ThreePointZero
  class ListingUnit
    ...
    storage_names[:default] = 'listing_unit'
    ...
    belongs_to :ssu,
               'NcsNavigator::Warehouse::Models::ThreePointZero::Ssu',
               :child_key => [ :ssu_id ], :required => false
    belongs_to :tsu,
               'NcsNavigator::Warehouse::Models::ThreePointZero::Tsu',
               :child_key => [ :tsu_id ], :required => false
    property   :list_line,
               NcsNavigator::Warehouse::DataMapper::NcsString,
               { :format => /^([-+]?[\d]{1,9})?$/ }
    property   :list_source,
               NcsNavigator::Warehouse::DataMapper::NcsString,
               { :required => true, :length => 1..2, :set => ["1", "2", "3", "-4"] }
    property   :list_comment,
               NcsNavigator::Warehouse::DataMapper::NcsText,
               { :pii => :possible, :length => 0..8000 }

    mdes_order :psu_id, :list_id, :ssu_id, :tsu_id, :list_line, :list_source, :list_comment
