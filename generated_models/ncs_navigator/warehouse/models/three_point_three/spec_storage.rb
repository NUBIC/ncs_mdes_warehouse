...
module NcsNavigator::Warehouse::Models::ThreePointThree
  class SpecStorage
    ...
    property   :temp_event_high_temp,
               NcsNavigator::Warehouse::DataMapper::NcsString,
               { :format => /^(([-+]?\d{1,12})|([-+]?\d{0,12}\.\d{1,6})|(\d{0,12}\.\d{1,6})|([-+]?\d{1,12}\.\d{0,6}))?$/ }
    property   :temp_event_comment,
               NcsNavigator::Warehouse::DataMapper::NcsString,
               { :pii => :possible, :length => 0..255 }

    mdes_order :psu_id, :spsc_id, :storage_container_id, :placed_in_storage_dt, :staff_id, :equip_id, :master_storage_unit, :storage_comment, :storage_comment_oth, :removed_from_storage, :removed_from_storage_dt, :temp_event_st, :temp_event_et, :temp_event_low_temp, :temp_event_high_temp, :temp_event_comment
